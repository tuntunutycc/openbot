...
- Anthropic pipeline fallback: when the Messages API fails with **credit/billing-style** errors (e.g. “credit balance too low”), `parse_caption` logs `Anthropic API credit low, using mock response.` and returns fixed mock `photoroom_background_prompt` / `marketing_copy` so Telegram + Photoroom steps can be tested without topping up; other API errors still surface to the user.
